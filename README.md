...
前后端分离的轻量级仓库管理系统，面向桌面端管理员与移动端作业人员双端场景。后端 Spring Boot 3 + H2/MySQL；前端 Vue 3 + Vite + Element Plus（响应式，桌面侧边栏 / 移动底部 Tab）。
...
- **货品管理**：SKU / 名称 / 规格 / 单位 → 增删改查，SKU 全局唯一

- **库位管理**：库位编码 / 区域 / 类型（常温/冷藏/大件/危险品/其他）/ 备注 → 编码唯一，存在库存时禁止删除

- **出库管理**：创建上架单（选择商品 + 计划数量）、状态筛选（待上架/已完成）、进度条、详情（含已上架明细）
